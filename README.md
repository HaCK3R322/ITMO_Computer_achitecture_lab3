...
| операции                   | объяснение                                                                             |
|----------------------------|----------------------------------------------------------------------------------------|
| <число>                    | Положить на верхушку стека число.                                                      |
...
| `ACCEPT`                   | Считать с порта ввода символ на верхушку стека                                         |
| `:function_name`           | Начать задавать функцию                                                                |
| `;`                        | Закончить задавать функцию                                                             |
| `IF` statements `THEN`     | Если на вершине не 0, то выполняем                                                     |
| `BEGIN` statements `UNTIL` | Выполняем как минимум один раз, если по результатам не 0, то заново                    |
...
- Ячейки с адресами 0x0000 -- 0xDF занимают таблицы переходов
...
### Стек возвратов
ну стек возвратов потом дописать кароч

## Набор инструкций

Так как имеется довольно жесткое ограничение в виде восьми битной памяти, а делать CISC архитектуру с командами
...
- команды разделены на адресные и безадресные:
  ...
  формату выглядят примерно вот так: `111xxxxx`, где `xxxxx` определяет OPCODE.
  - адресные команды выглядят вот так: `YYYxxxxx`, где `YYY` будет определять OPCODE инструкции. 
  - `LOAD`, `CALL` и `JMPA` это прямая адресация.
  - `JMPR`, `JZ`, `JL`, `JO` это относительная.
  - При прямой адресации addr выступает офсетом в таблице адресов для этой команды.
  - При относительной адрес складывается (или не складывается) с PC и получается "прыжок"
  

Конечно, такая архитектура может показаться немного сложной, но я рассудил это наиболее оптимальным способом распорядится
восьмибитной команд.

| Команда    | Адресная?                   | Тип           | Кол-во тактов <br/>(выполнение) | Влияние на стек/память        | Описание                                                                                                          |
|------------|-----------------------------|---------------|---------------------------------|-------------------------------|-------------------------------------------------------------------------------------------------------------------|
| LOAD addr  | Да                          | memory access | +                               | a b -- a b MEM[addr]          | Загрузить на стек значение по адресу                                                                              |
| JMPA  addr | Да                          | flow control  | +                               | -                             | Прыгнуть на команду по адресу                                                                                     |
| JMPR  addr | Да, относительная адресация | flow control  | +                               | -                             | Перепрыгнуть через addr адресов                                                                                   |
| JZ   addr  | Да, относительная адресация | flow control  | -                               | -                             | Перепрыгнуть через addr адресов, если ZF==1                                                                       |
| JL   addr  | Да, относительная адресация | flow control  | -                               | -                             | Перепрыгнуть через addr адресов, если NF==1                                                                       |
| JO   addr  | Да, относительная адресация | flow control  | -                               | -                             | Перепрыгнуть через addr адресов, если OF==1                                                                       |
| CALL addr  | Да                          | flow control  | +                               | -                             | Вызвать функцию по её адресу                                                                                      |
| RET        | Нет                         | flow control  | +                               | -                             | Вернуться из функции                                                                                              |
| TRUE       | Нет                         | logic         | +                               | a b -- b a -1                 | Положить на вершину стека `True` (0)                                                                              |
| FALSE      | Нет                         | logic         | +                               | a b -- b a 0                  | Положить на вершину стека `False` (-1)                                                                            |
| CMP        | Нет                         | logic         | +                               | -                             | Установить флаги по результатам сравнения двух верхних чисел. (ZF/NF: a == b -> 1/0; a > b -> 0/0; a < b -> 0/1 ) |
| SWAP       | Нет                         | logic         | +                               | a b -- b a                    | Поменять местами два верхних элемента стека                                                                       |
| OVER       | Нет                         | logic         | +                               | a b -- a b a                  | Продублировать предпоследний элемент стека                                                                        |
| DUP        | Нет                         | logic         | +                               | a b -- a b b                  | Продублировать последний элемент стека                                                                            |
| DROP       | Нет                         | logic         | +                               | a b -- a                      | Выкинуть вершину стека                                                                                            |
| ROT        | Нет                         | logic         | +                               | a b c -- b c a                | Ротация 3х верхних элементов                                                                                      |
| TOR        | Нет                         | logic         | +                               | S: a R: -- S: R: a            | Запушить TOS в RETURN STACK                                                                                       |
| RFROM      | Нет                         | logic         | +                               | S: R: a -- S: a R:            | Запушить вершину RETURN STACK'a в STACK                                                                           |
| SET        | Нет                         | memory access | +                               | a b c d -- a; MEM[cd]=b       | Сохранить значение ( 8 бит) в память, используя 2 верхних элемента как адрес                                      |
| GET        | Нет                         | memory access | +                               | a b c -- a MEM[bc]            | Загрузить в стек значение ( 8 бит) из памяти, используя 2 верхних элемента как адрес                              |
| SUM        | Нет                         | arithmetic    | +                               | a b -- a+b                    | 8-битное сложение                                                                                                 |
| SUB        | Нет                         | arithmetic    | +                               | a b -- a-b                    | 8-битное вычитание                                                                                                |
| DIV        | Нет                         | arithmetic    | +                               | a b -- a/b                    | 8-битное деление                                                                                                  |
| MUL        | Нет                         | arithmetic    | +                               | a b -- a*b                    | 8-битное умножение                                                                                                |
| MOD        | Нет                         | arithmetic    | +                               | a b -- a%b                    | 8-битное деление по модулю                                                                                        |
| INC        | Нет                         | arithmetic    | +                               | a b -- a ++b                  | 8-битное инкремент                                                                                                |
| DEC        | Нет                         | arithmetic    | +                               | a b -- a --b                  | 8-битное декремент                                                                                                |
| READ       | Нет                         | IO            | -                               | a b -- a b new_symbol         | Считать один символ с порта на вершину стека                                                                      |
| PRINT      | Нет                         | IO            | +                               | a b -- a ; out_port.append(b) | Вывести вершину стека на порт                                                                                     |
...
- В случае адресных инструкций, может иметься offset 
...
- `offset` -- оффсет адресной инструкции
