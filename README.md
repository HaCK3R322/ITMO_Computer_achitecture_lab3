...
- Реализуется списком словарей формата либо `{value: OPCODE, related_token_index: INDEX}` для безадресных, либо
`{value: OPCODE, address: ADDRESS, related_token_index: INDEX}` для адресных
...
- команды разделены на адресные и безадресные:
  ...
  по совместительству адрес указателя на таблицу смещения под инструкцию / 8, а `xxxxx` будет 
  представлять собой смещение по таблице адресов переходов для этой конкретной инструкции.

Конечно, такая архитектура может показаться немного сложной, но я считаю что она позволяет достаточно эффективно 
распорядиться 8-битной памятью, позволяя процессору работать с 8ью адресными инструкциями, каждая из которых может иметь
до 32х различных вариаций переходов, а также до 32х безадресных команд.
...
| SAVE addr | Да        | memory access |                                 | a b -- a; MEM[addr] = b | Выгрузить вершину стека в ячейку по адресу                                           |
| JMP  addr | Да        | flow control  |                                 | -                       | Прыгнуть на команду по адресу                                                        |
| JZ   addr | Да        | flow control  |                                 | -                       | Прыгнуть на команду по адресу, если ZF==1                                            |
| JNZ  addr | Да        | flow control  |                                 | -                       | Прыгнуть на команду по адресу, если ZF==0                                            |
