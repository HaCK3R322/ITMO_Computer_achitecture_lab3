...
- команды разделены на адресные и безадресные:
  ...
  - адресные команды выглядят вот так: `YYYxxxxx`, где `YYY` будет определять OPCODE инструкции и 
  по совместительству адрес указателя на таблицу смещения под инструкцию (`0b0..0YYYxxxxx`), а `xxxxx` будет 
  представлять собой смещение по таблице адресов переходов для этой конкретной инструкции (выделяется только
  LOAD инструкция - `00xxxxxx`, до 64 смещений вместо 32). 
  - SUMMARY: по сути имеется по 32 варианта "куда прыгнуть" для каждого из [`JMP`, `JZ`, `JL`, `JO`], можно выгрузить из
  памяти до 64 адресов различных ячеек с помощью `LOAD`, а также иметь 32 различных метки для функций

Конечно, такая архитектура может показаться немного сложной, но я считаю что она позволяет достаточно эффективно 
распорядиться 8-битной памятью, позволяя процессору работать с 6ью адресными и 32мя безадресными инструкциями.
...
| JMP  addr | Да        | flow control  | +                               | -                       | Прыгнуть на команду по адресу                                                        |
| JZ   addr | Да        | flow control  | +                               | -                       | Прыгнуть на команду по адресу, если ZF==1                                            |
| JL   addr | Да        | flow control  | +                               | -                       | Прыгнуть на команду по адресу, если NF==1                                            |
| JO   addr | Да        | flow control  | +                               | -                       | Прыгнуть на команду по адресу, если OF==1                                            |
...
| ADD       | Нет       | arithmetic    | +                               | a b -- a+b              | 8-битное сложение                                                                    |
